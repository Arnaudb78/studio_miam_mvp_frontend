...
export default function CardPayementDetails({ id }: DetailsClientProps) {
    ...
    useEffect(() => {
        const fetchAppartDetails = async () => {
            try {
                const response = await fetch(`https://pacific-reaches-55510-1cc818501846.herok/apparts/${id}`);
                if (!response.ok) {
        ...
        const fetchUserInfo = async (userId: string) => {
            try {
                const response = await fetch(`https://pacific-reaches-55510-1cc818501846.herok/users/${userId}`);
                if (!response.ok) {
            ...
            <section className="p-2 w-full h-full flex flex-col gap-8 font-satoshi lg:p-8">
                ...
                <div className="w-full h-full bg-[#F4F3EB] p-8 rounded-2xl flex flex-col gap-4">
                    ...
                    <div className="my-2 flex gap-4 opacity-60">
                        <input type="checkbox" id="saveCard" checked={saveCard} onChange={handleSaveCardChange} className="w-6 h-6" />
                        <label htmlFor="saveCard">Sauvegarder la carte</label>
                    </div>
                    <button onClick={handlePayement} className="w-full p-3 bg-primary text-secondary-200 rounded-lg">
                        Payer {data.price}€
                    </button>
                    <p className="text-[10px] opacity-60">
                        Vos données personnelles seront utilisées pour traiter votre commande, soutenir votre expérience sur ce site Web et à d’autres
