        ...
        <div>
            ...
            <div className="flex flex-col gap-8 p-2 font-satoshi text-[13px]">
                ...
                <div className="flex flex-col bg-primary text-secondary-200 rounded-2xl p-4 gap-4 mt-10">
                    <h2 className="text-[22px]">La map pour retrouver le <span className="bg-secondary-300 text-secondary-200 rotate-3 inline-block">chemin</span> de vos désirs !</h2>
                    <p className="opacity-75">Ce charmant appartement fonctionnel disposant d'un balcon donnant
                         côté cour est parfait pour découvrir Paris. Dans un quartier animé
                          (nombreux bars proches du canal) et avec de nombreux commerces, l'appartement
                           est pourtant parfaitement au calme.</p>
                    <p className="text-[16px]">{data.localisation.address} {data.localisation.zip_code} {data.localisation.city}</p>
                ...
                <div className="w-full h-full bg-[#F4F3EB] p-4 flex flex-col gap-1 rounded-2xl">
                    <p className="text-[20px] my-10">Du chaos familial au <span className="bg-secondary-100 text-secondary-200 rotate-3 inline-block">paradis</span> romantique, découvrez ce qu'ils en disent !</p>
                    {comments}
                </div>
                <div className="bg-transparent backdrop-blur-lg fixed flex justify-center items-center p-8 w-full bottom-0 left-0">
                    <button onClick={handleClick} className="bg-secondary-300 p-4 text-secondary-200 rounded-full">Pré-réserver en vitfesse</button>
                </div>
                {/* <h1>{data.title}</h1>
                <p>{data.description}</p>
                <p>
